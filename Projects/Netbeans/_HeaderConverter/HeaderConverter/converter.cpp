...
void almacenarProximaKeyWord(ifstream &archFuente,char *keyWord){
    // Declaracion de Variables
    bool KwCoincidentes[med_KW]{},haySimilares,huboDescarte;
    int posExacta = -1; char letra;
    ...
    for (int i = 0;1;i++){
        ...
        if (not haySimilares){
            if(posExacta != -1){
                if(huboDescarte or esElemento(letra,modificadores)){
                    if(esElemento(letra,modificadores)) archFuente.unget();
                    strcpy(keyWord,keyWords[posExacta].identificador);
                    agregarProximosModificadores(archFuente,keyWord);
                    return;
                } else {
                    posExacta = -1;
                }
