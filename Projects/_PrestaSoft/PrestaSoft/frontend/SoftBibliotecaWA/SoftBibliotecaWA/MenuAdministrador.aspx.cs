...
namespace SoftBibliotecaWA
{
    public partial class MenuAdministrador : System.Web.UI.Page
    {
        ...
        private void cargarPoliticas()
        {
            ...
            //BindPoliticasDropDown(); //ya no existe
        ...
        private void cargarUniversidades()
        {
            ...
            BindingList<universidad> universidades = universidadBO.listarTodos(-1, -1, -1); //se supone que lista hasta los consorcios y políticas                                                                            // Definir objetos dummy
        ...
        */
        ...
        //SECCIÓN CONSORCIO
        protected void btnAbrirModalConsorcio_Click(object sender, EventArgs e)
        {
            LimpiarCamposConsorcio();
            Session["accionConsorcio"] = "Insertar";
            Session.Remove("idConsorcio");
            modalConsorcioLabel.InnerText = "Ingresar Consorcio";

            // Abrir el modal utilizando ClientScript
            string script = "window.onload = function() { showModalConsorcio(); };";
            ClientScript.RegisterStartupScript(this.GetType(), "ShowModalScript", script, true);
        }
        protected void lbEditarConsorcio_Click(object sender, EventArgs e)
        {
            if (!int.TryParse(((LinkButton)sender).CommandArgument, out int idConsorcio))
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorFormatoIDEditar", "alert('El ID del consorcio no tiene un formato válido.');", true);
                return;
            }

            Session["accionConsorcio"] = "Editar";
            Session["idConsorcio"] = idConsorcio;

            CargarDatosConsorcio(idConsorcio);
            modalConsorcioLabel.InnerText = "Editar Consorcio";

            // Abrir el modal utilizando ClientScript
            string script = "window.onload = function() { showModalConsorcio(); };";
            ClientScript.RegisterStartupScript(this.GetType(), "ShowModalScript", script, true);
        }
        private void CargarDatosConsorcio(int idConsorcio)
        {
            consorcioBO = new ConsorcioBO();
            consorcio consorcio = consorcioBO.obtenerPorId(idConsorcio);

            if (consorcio != null)
            {
                txtCIFConsorcio.Text = consorcio.CIF;
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorCargaDatos", "alert('Error: No se pudieron cargar los datos del consorcio.');", true);
            }
        }
        protected void btnGuardarConsorcio_Click(object sender, EventArgs e)
        {
            string accion = Session["accionConsorcio"] as string;

            // Obtener y validar los valores ingresados
            string CIF = txtCIFConsorcio.Text;

            if (string.IsNullOrEmpty(CIF))
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorValidacionCIF", "alert('Por favor, ingresa un valor válido para CIF.');", true);
                return;
            }
            ...
            try
            {
                if (accion == "Insertar")
                {
                    ...
                    consorcioBO.insertar(CIF);
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "SuccessInsertar", "alert('Consorcio registrado correctamente.');", true);
                }
                else if (accion == "Editar")
                {
                    if (Session["idConsorcio"] != null && int.TryParse(Session["idConsorcio"].ToString(), out int idConsorcio))
                    {
                        consorcioBO.modificar(idConsorcio, CIF);
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "SuccessEditar", "alert('Consorcio actualizado correctamente.');", true);
                    }
                    else
                    {
                        // Manejar el caso en que el ID no esté disponible
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorID", "alert('Error: No se pudo obtener el ID del consorcio de la sesión.');", true);
                        return;
                    }
                }

                // Limpiar variables de sesión
                Session.Remove("accionConsorcio");
                Session.Remove("idConsorcio");

                // Recargar la lista de consorcios
                cargarConsorcio();

                // Cerrar el modal utilizando ClientScript
                string script = "window.onload = function() { hideModalConsorcio(); };";
                ClientScript.RegisterStartupScript(this.GetType(), "HideModalScript", script, true);
            }
            catch (Exception ex)
            {
                // Manejar errores durante la operación
                string errorMessage = ex.Message.Replace("'", "\\'");
                string scriptError = $"alert('Error al guardar el consorcio: {errorMessage}');";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorGuardarConsorcio", scriptError, true);
            }
        }
        protected void btnCancelarConsorcio_Click(object sender, EventArgs e)
        {
            // Limpiar variables de sesión y campos
            Session.Remove("accionConsorcio");
            Session.Remove("idConsorcio");
        ...
        private void LimpiarCamposConsorcio()
        {
            txtCIFConsorcio.Text = "";
        }
        protected void lbEliminarConsorcio_Click(object sender, EventArgs e)
        {
            if (!int.TryParse(((LinkButton)sender).CommandArgument, out int idConsorcio))
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorFormatoIDEliminar", "alert('El ID del consorcio no tiene un formato válido.');", true);
                return;
            }
            ...
            try
            {
                int resultado = consorcioBO.eliminar(idConsorcio);

                if (resultado < 0)
                {
                    // Mostrar mensaje de error
                    string scriptError = "alert('Error al eliminar el consorcio, posiblemente hay universidades relacionadas.');";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorEliminacion", scriptError, true);
                }
                else
                {
                    // Mostrar mensaje de éxito
                    string scriptSuccess = "alert('Consorcio eliminado correctamente.');";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "EliminacionExitosa", scriptSuccess, true);
                }
            ...
            catch (Exception ex)
            {
                // Mostrar mensaje de error con interpolación correcta
                string errorMessage = ex.Message.Replace("'", "\\'");
                string scriptError = $"alert('Error al eliminar el consorcio: {errorMessage}');";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ErrorEliminacion", scriptError, true);
